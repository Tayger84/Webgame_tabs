...
def parse_alliance_file(html_content):
    soup = BeautifulSoup(html_content, 'lxml')
    
    # find the header for check correct page 
    header = soup.find(['h1', 'h2', 'h3'], string=lambda t: t and "Detaily zemí aliance" in t)
    if not header:
        return None
...
with open("NTRLTY_aliance.html", "r") as f:
    html = f.read()
    
    overview = parse_alliance_overview(html) 
    
print(overview)   
    
with open("NTRLTY_aliance_detaily.html", "r") as t:
    html = t.read()
    detail = parse_alliance_file(html)
